...
import type {FxApiResponse} from '@infrastructure/currency/fx-types';
...
describe('FxService', () => {
  describe('fetchAndCacheRates', () => {
    it('should fetch rates from API and save them to the repository', async () => {
      ...
      const {makeFetchAndCacheRates} = require('@infrastructure/currency/fx-service');
    ...
    it('should delete stale rates older than 24 hours', async () => {
      ...
      const {makeFetchAndCacheRates, STALE_THRESHOLD_MS} = require('@infrastructure/currency/fx-service');
      const execute = makeFetchAndCacheRates({
        fxRateRepo: repo,
        fetchRates: mockFetch,
      });
  ...
  describe('getConversionRate', () => {
    it('should return 1 for same currency', async () => {
      ...
      const {makeGetConversionRate} = require('@infrastructure/currency/fx-service');
  ...
  describe('convertAmount', () => {
    it('should convert cents using the rate', () => {
      const {convertAmountCents} = require('@infrastructure/currency/fx-service');
